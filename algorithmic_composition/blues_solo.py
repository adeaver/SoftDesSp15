...
def add_note(out, instr, key_num, duration, bpm, volume):
    ...
    try:
        ...
        stream = instr.play(abs(duration*(60.0/bpm)),freq)
        stream *= volume
        out << stream
    except:
        print "Input Duration: " + str(duration) + " -- Error Avoided"
...
beats_per_minute = 45.0				# Let's make a slow blues solo

curr_note = 0
add_note(solo, bass, blues_scale[curr_note], 1.0, beats_per_minute, 1.0)

licks = [ [-1, 0.25], [3, 0.5], [-2, 0.15], [1, 0.15], [-4, 0.2], [7, 0.3], [9, .1], [-2, 1]]
for i in range(20):
    note = licks[random.randrange(0, len(licks))]
    curr_note += note[0]
    if(curr_note < 0):
        curr_note = 0
...
solo *= 1.2            # adjust relative volumes to taste
backing_track *= 2.0
